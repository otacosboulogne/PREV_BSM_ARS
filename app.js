(() => {
  ...
  async function importExcel(file){
    ...
    try{
      ...
      $('forecastCAValue').textContent = parsed.ca > 0 ? money0(parsed.ca) : '—';
  ...
  function loadRealDay(id){
    ...
    $('actualForecastCA').textContent = num(realDay.forecastCA) > 0 ? money0(realDay.forecastCA) : '—';
  ...
  async function importCaForecastExcel(file){
    ...
    try{
      ...
      const wb=XLSX.read(data,{type:'array',cellDates:false,cellFormula:false,cellStyles:false});
  ...
  function renderCaArchive(){
    ...
    tb.innerHTML=rows.length
      ? rows.map(r=>`<tr><td>${formatDateFR(r.date)}</td><td class="num-strong">${money0(r.forecastCA)}</td></tr>`).join('')
  ...
  function renderAnalysisKpis(days, rows){
    ...
    $('kpiForecastCA').textContent = forecastCA > 0 ? money0(forecastCA) : '—';
  ...
  function buildPdf(day, kind){
    ...
    const compactForecast = kind !== 'closure';
    let y = 12;
    ...
    y = pdfTable(doc, y, ['Produit','MIDI','SOIR','Total'], productionRows, {firstColWide:true, orange, black, soft, compact:compactForecast});

    y += compactForecast ? 2 : 4;
    ...
    y = pdfTable(doc, y,
      ...
      {firstColWide:true, orange, black, soft, checkboxCols:[2,4], compact:compactForecast}
    );
    ...
    y = pdfTable(doc, y, ['Produit','Marge MIDI','Réel total MIDI','Marge SOIR','Réel total SOIR'], marginRows, {firstColWide:true, orange, black, soft, compact:compactForecast});
  ...
  function pdfTable(doc, y, head, body, opts){
    ...
    const fontSize = opts.fontSize || (compact ? 7.6 : 8.2);
    const cellPadding = compact ? 1.25 : 2.15;
    ...
    doc.autoTable({
      ...
      body,
      theme:'grid',
      margin:{left:12,right:12,bottom:compact ? 12 : 16},
      styles:{font:'helvetica',fontSize,cellPadding,textColor:opts.black,lineColor:[218,218,218],lineWidth:.18,halign:'center',valign:'middle',minCellHeight:compact ? 5.1 : 0},
  ...
  function money0(v){ return num(v).toLocaleString('fr-FR',{style:'currency',currency:'EUR',minimumFractionDigits:0,maximumFractionDigits:0}); }
